                ...
                @foreach($units as $unit)
                    <tr>
                        ...
                        <td class="py-2 px-4 border-b text-center">
                            <a href="{{ url('eenheden/' . $unit->id . '/edit') }}" class="bg-blue-600 hover:bg-blue-300 text-yellow-50 py-1 px-1 rounded">Pas eenheid aan</a>
                            <form action="{{ url('eenheden/' . $unit->id) }}" method="post" class="inline-block">
                                @csrf
                                @method('delete')
                                <button type="submit" class="bg-red-600 hover:bg-red-500 text-yellow-50 py-1 px-1 rounded">Verwijder eenheid</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
