...
<body class="bg-yellow-50">
    <form action="{{ url('pizza/' . $pizza->id) }}" class="max-w-md mx-auto bg-neutral-900 p-6 rounded-lg mt-10"
        method="POST">
        @csrf
        @method('put')
        <div class="mb-5">
            <label class="block text-yellow-50 font-bold mb-2" for="name">Pizza</label>
            <input type="text" id="name" name="name" value=" {{ $pizza->name }} " placeholder="name"
        ...
        <div class="mb-5">
            ...
            <textarea id="beschrijving" name="beschrijving" placeholder="beschrijving"
                class="border-black border rounded w-full py-2 px-3 bg-neutral-200 resize-y overflow-hidden">{{ $pizza->beschrijving }}</textarea>
        </div>
        <div class="mb-5">
            <label class="block text-yellow-50 font-bold mb-2" for="prijs">Prijs</label>
            <textarea id="prijs" name="prijs" placeholder="prijs"
                class="border-black border rounded w-full py-2 px-3 bg-neutral-200 resize-y overflow-hidden">{{ $pizza->prijs }}</textarea>
        </div>
