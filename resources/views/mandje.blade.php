                    ...
                    @if (Route::has('login'))
                            @auth
                                <a href="{{ url('/dashboard') }}"
                                    class="font-semibold bg-orange-400 text-white rounded-full px-4 py-2 hover:text-orange-300 ml-1">Account</a>
                            @else
                                <a href="{{ route('login') }}"
                                    class="font-semibold text-orange-400 hover:text-orange-300 ml-1">Inloggen</a>

                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}"
                                        class="font-semibold bg-orange-400 text-white rounded-full px-4 py-2 hover:text-orange-300 ml-1">Registreren</a>
                                @endif
                            @endauth
                        </div>
                    @endif
    ...
    {{--<section class="flex flex-wrap lg:flex-nowrap">
            ...
            <div>
                ...
                <div class="mt-4">
                    ...
                    <select id="ingredient1" name="ingredient1"
                        onchange="document.getElementById('ingredienteen').value = document.getElementById('ingredient1').value">
                    ...
                    <select id="ingredient2" name="ingredient2"
                        onchange="document.getElementById('ingredienttwee').value = document.getElementById('ingredient2').value">
                        @foreach ($ingredienten as $ingredient)
                        <option value="{{ $ingredient->id }}">{{ $ingredient->name }}</option>
                        @endforeach
                    </select>
                ...
                <form action="{{ url('mandje/' . $item->id) }}" method="post">
                    ...
                    @method('delete')
                    <button type="submit"
                        class="bg-red-600 hover:bg-red-500 text-yellow-50 py-2 px-4 mt-5 rounded-full">Verwijder van
                        bestelling</button>
                </form>
            </div>
            @endforeach
        ...
        <div class="w-full lg:w-1/3 md:w-full bg-slate-100 m-10 p-10 rounded-md shadow-md">
            ...
            <form action="mandje" method="post">
                ...
                <button type="submit"
                    class="bg-orange-400 hover:bg-orange-300 text-white py-2 px-4 mt-5 mb-5 rounded-full">Bereken
                    prijs</button>
                <br>
                <a href="{{ url('/status') }}"
                    class="bg-orange-400 text-white rounded-full px-4 py-2 hover:bg-orange-300">Bestellen</a>
            </form>
            @else
            ...
            <div class="flex flex-wrap gap-10">
                @foreach($order as $item)
                    <div class="mt-5">
                        <h1 class="font-bold">{{ $item->name }}</h1>
                        <div class="mt-4">
                            <label class="block text-black mb-2" for="grootte_{{ $item->id }}">Kies grootte van de
                                pizza</label>
                            <select id="grootte_{{ $item->id }}" name="grootte_{{ $item->id }}"
                                onchange="document.getElementById('Grootte').value = document.getElementById('grootte_{{ $item->id }}').value">
                                @foreach ($groottes as $grootte)
                                    <option value="{{ $grootte->id }}">{{ $grootte->name }}</option>
                                @endforeach
                            </select>
        ...
        <div class="w-full lg:w-1/3 md:w-full bg-slate-100 m-10 p-10 rounded-md shadow-md">
            ...
            @if (!empty($grootte))
                ...
                <form action="/bestel" method="post">
                    ...
                    @endforeach
                    <button type="submit"
                        class="bg-orange-400 hover:bg-orange-300 text-white py-2 px-4 rounded-full">Bestellen</button>
                </form>
